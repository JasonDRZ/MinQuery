...
const rootMinQuery = function (pageName, recoveryMode) {
	 ...
	 */
	MinQuery.wxMethod = wxMethods.wxMethodsCaller;
	...
	MinQuery.extend({
		$apply(_tar, argsArr) {
			MinQuery.isFunction(_tar) && $tools.carry(this, _tar, argsArr);
		},
		$call() {
			let args = $tools.slice.call(arguments), _tar = args[0];
			args.shift();
			MinQuery.isFunction(_tar) && $tools.carry(this, _tar, args);
		}
	})
					...
					if (this.$selectorName === "page" || this.$selectorName === "app") {
						...
						if (_type in MinQuery.pageInheritEventKVPair) {
							...
							if (_type in this && MinQuery.isFunction(this[_type])) {
								res = this[_type](data);
								MinQuery.isFunction(triggerCall) && $tools.carry(null, triggerCall, {
									"$data": data,
									"$res": res
								});
		...
		off(_type, offCall, iscatch) {
			...
			if (!!this && this.length > 0) {
				...
				this.each(function () {
					...
					MinQuery.isFunction(offCall) && $tools.carry(this, offCall);
				})
			}
			return this;
		},
		...
		get: function (elekeys, eventkeys, eventdata, triggerdata) {
			...
			if (!!eventObj && !!eventObj.active) {
				...
				!!regObj && regObj.method && MinQuery.each(regObj.method, (i, reg) => {
					MinQuery.isFunction(reg) && $tools.carry(null, function () {
						reg.apply(eleContext, dataArr);
					});
				});
				
				let _method_return;
				_method_return = MinQuery.isFunction(eventObj.method) ? $tools.carry(null, function () {
		...
		timeOut(call, delay, still) {
			return setTimeout(function () {
				if (!!this.still || !this.still && MinQuery.isReady) {
					MinQuery.isFunction(this.call) && $tools.carry(null, this.call);
				}
			}.bind({
				call: call,
				// 标识是否检测ready字段
	...
	MinQuery.fn.extend({
		...
		hasClass(className, hover) {
			...
			for (; i < len;) {
				ele = this[i++];
				if (MinQuery.isFunction(className)) className = $tools.carry(ele, className, ele);
		...
		addClass(className, hover) {
			this.each(function () {
				if (MinQuery.isFunction(className)) className = $tools.carry(this, className, this);
