...
const rootMinQuery = function (pageName, recoveryMode) {
	...
	MinQuery.extend =
		//原型集成方法
		MinQuery.fn.extend = $tools.extend;
	...
	MinQuery.extend(MinQuery, {
			...
			/**
			 * page initial object 页面实例对象初始化数据结构
 			 */
			pageInitObject: pageName == 'app' ? {
				"$selectorType": "$app",
				"$selectorName": "app",
				"data": {
			 ...
			 * @param call 执行回调
			 * @param delay 延时时长
			 * @param still 只有在当前页面初始化完成才会执行回调
			 * @return {number}
			 */
			 ...
			 */
			wxMethod: wxMethods._wxMethodsThenTransformer,
			/**
			 * 定义模块
			 * @type {defineModule}
			 */
			definedModule: $global.$module.defineModule,
			/**
			 * 调用模块
			 * @type {callModule}
			 */
			module: $global.$module.callModule,
			/**
			 * 获取已经开启的页面实例对象
			 * @param pageName
			 */
			 ...
			 */
			$off(event, filterArr) {
				$global.$events.destroy(MinQuery.pageName, event, filterArr);
			},
			/**
			 * 全局事件广播
			 *
		 ...
		 * 微信封装接口
		 */
		wxMethods.wxMethodsPackages,
		/**
		 * 样式操作管理器
		 */
		$tools.styleController,
		/**
		 * 核心工具方法集
		 */
		$tools);
	...
	/**
	 * 数据快捷操作方法集
	 * @type {{get: ((key?)), set: ((key?, value?)), clear: ((key?, _type?)), append: ((value?, isBatch)), prepend: ((value?, isBatch)), after: ((index?, value?, isBatch)), before: ((index?, value?, isBatch)), remove: ((key?, index?))}}
	 * @private
	 */
	let __dataHooks__ = {
		// 获取
	...
	let setCurrentPageData = function (keyString, keyValue, stayFormat, detecteInerit) {
		...
		if (!$tools.isEmptyObject(keyString)) {
			...
			// 小程序原型方法，对App页面禁用此方法
			!!MinQuery.pageInstance && MinQuery.pageName !== 'app' && MinQuery.pageInstance.setData(keyString);
			// 同步更新框架上的数据
	 ...
	 * @param _c_e_name 事件名称
	 * @param _middleware 事件中间件处理器
	 */
	...
	if (MinQuery.pageName === "app") {
		...
		function ghostEvent (e,ename){
			var _rewrite_event = {};
			ename === MinQuery.pageInheritEventKVPair.launch || ename === MinQuery.pageInheritEventKVPair.show && $tools.isPlainObject(e)
				...
				//设置querys的值
				MinQuery.querys = _rewrite_event,
				// 设置ready标示
				MinQuery.isReady = true) : (_rewrite_event = e);
			return _rewrite_event;
		}
		pageEventMiddleware = function (e, ename) {
			e = ghostEvent(e, ename);
			...
			// 先去掉对APP页面的对象删除，防止二次进入APP出现对象失联的情况
			// if (ename === MinQuery.pageInheritEventKVPair.hide) {
			// 	// 离开页面时，回收所有框架注册对象数据
			...
			let ret = eventHooks.get(`${$mq_config.selectorsBank.app[0]}.app`, `bind.${MinQuery.pageInheritEventKVPair[ename]}`, e);
			// 未查询到自定义事件是执行查询原始事件名称事件
			if (ret === '[No-Event-Handler]') {
				// 查询执行元素原生事件
				return eventHooks.get(`${$mq_config.selectorsBank.app[0]}.app`, `bind.${ename}`, e);
			} else {
				return ret;
			}
		}
		...
		pageEventMiddleware = function (e, ename) {
			...
			if (ename === MinQuery.pageInheritEventKVPair.load) {
				...
				/**
				 * 将load事件中传出的路径参数绑定到框架上
				 */
				MinQuery.querys = e;
				//将Page实例化的pageInstance镜像到pageInitObject上，是两个对象尽可能保持一致
	...
	/**
	 * 元素对象查询器，用户查询对应的元素对象，如果没有对象则生成一个元素对象
	 * @param selector
	 * @return {*}
	 */
	...
	/**
	 * Page主选择器，用于选择元素、PageInstance和APPInstance对象等。
	 * 支持多元素对象查询方法：
	 * $('.eleOne,#eleTwo');
	 * 获取框架封装的AppInstance，这样获取的实例对象允许使用框架方法绑定和操作数据，而$.$app()获取到的则是原始APP实例对象数据，无法使用框架方法进行事件和数据操作。
	 * $('app');
	 * PageInstance实例对象获取，在对应的页面获取的仅是当前页面的封装实例对象，若要获取其他页面的实例对象，请参考并使用$.$page('pageName')方法进行获取。
	 * $('page');
	 *
	 * @param selector
	 * @return {*}
	...
	// 操作元素的私有属性方法，此设置不参与视图层更新操作
	let elem_priv = {
		priv_keys: "$__priv_keys__",
		/**
		 * 获取私有数据；获取时，允许传入一个预设值，当获取的数据不存在的时候进行预设。
		 * @param ele 元素对象
		 * @param _type 私有属性名
		 * @param value 预设值【当没有查询到响应值时，将会将此值作为初始值设置并使用】
		 * @param arrPush 是否是数组压栈
		 * @return {*}
		 */
		get(ele, _type, value, arrPush) {
			let cur_priv = MinQuery.getData(ele, `${this.priv_keys}.${_type}`);
			//将文设置webviewId的context对象设置当前的webviewid
			if ($tools.isPlainObject(cur_priv) && cur_priv.hasOwnProperty('webviewId') && $tools.isUndefined(cur_priv.webviewId)) {
	...
	MinQuery.fn.extend({
		...
		/**
		 * 访问、设置私有属性数据，此方法不参数视图更新
		 * @param key 键
		 * @param value 值
		 * @param arrPush 是否数组数据压栈
		 * @return {*}
		 */
		...
		/**
		 * config方法与attr方法的使用相同的，主要差别在应用场景上。此方法用与管理组件的配置信息，attr用于管理元素的属性设置
		 * @param key
		 * @param value
		 * @return {*}
		 */
		...
		/**
		 * animation delay，对原生animation和cssAnimation方法均有效
		 * @param time
		 * @return {delay}
		 */
		delay(time) {
		...
		/**
		 * animation step setting，原生动画对象设置方法
		 * @param animateStepFunc
		 * @param speed
		 * @param easing
		 * @param origin
		 * @return {animation}
		...
		/**
		 * 基于css创建动画
		 * @param stylesGetFunc 接收一个返回style字符串或是对象的匿名函数，或是直接的style字符串或对象
		 * @param speed 动画执行时长，以毫秒计算，整数形式，默认200ms
		 * @param bezier 支持贝塞尔曲线函数，字符串形式，不填则默认ease
		 * @param targetStyleArr 需要进行动画过度的目标样式数组，如：height；没有则默认all
		 * @return {cssAnimation}
		 */
		 ...
		 * @param key
		 * @param value
		 * @param animationKey 此字段用于扩展是cssAnimation方法能使用此犯法进行样式设置
		 * @return {*}
		 */
		...
		/**
		 * 检测某个样式是否存在，此方法仅能检测通过框架管理的样式名称，不能检测元素初始设置的样式
		 * @param className
		 * @param hover 是否hover样式
		 * @return {boolean}
		 */
